...
```
  ...
  <done>Task result interfaces and Zod schemas added for all 6 task types</done>
</task>
...
```
  ...
  <done>TaskTypeMatrix with prompts, parsers, and fallback handling for all 6 task types</done>
</task>
...
```
  ...
  <done>AiGatewayService enhances prompts and parses responses using TaskTypeMatrix</done>
</task>
...
```
  ...
  <done>AiController has /execute endpoint; AiModule provides all AI services</done>
</task>
...
```</action>
  ...
  <done>TaskTypeMatrix exported from ai module</done>
</task>
