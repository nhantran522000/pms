...
```
  ...
  <done>CircuitBreakerService with isBlocked(), recordFailure(), recordSuccess() methods; 60s block for 429, immediate rotation for 5xx</done>
</task>
...
```</action>
  ...
  <done>AiGatewayService checks circuit breaker before trying each provider; records failures and successes</done>
</task>
...
```</action>
  ...
  <done>CircuitBreakerService exported from ai module index</done>
</task>
